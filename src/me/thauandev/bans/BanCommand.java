		...
		if (cmd.getName().equalsIgnoreCase("ban")) {
			...
			if (vitima == null) {
				...
				if (Config.getConfig().getBans().get("Bans." + vitimaoff.getUniqueId()) != null) {
					API.sendMsg(jogador,
							"§c§lSPEED§f§lMC §fO jogador §c§l" + vitimaoff.getName() + " §fjá está §4§lBANIDO§f!");
					API.sendSound(jogador, Sound.NOTE_PLING, 10);
					return true;
				}
				if (Config.getConfig().getIpBans().get("IPBans." + vitimaoff.getUniqueId()) != null) {
					API.sendMsg(jogador, "§c§lSPEED§f§lMC §fO jogador §c§l" + vitimaoff.getName()
							+ " §fjá está §4§lBANIDO§4(IP)§f!");
					API.sendSound(jogador, Sound.NOTE_PLING, 10);
					return true;
				}
				if (Config.getConfig().getTempBans().get("TempBans." + vitimaoff.getUniqueId()) != null) {
					API.sendMsg(jogador, "§c§lSPEED§f§lMC §fO jogador §c§l" + vitimaoff.getName()
							+ " §fjá está §9§lBANIDO§9(tempban)§f!");
					API.sendSound(jogador, Sound.NOTE_PLING, 10);
					return true;
				...
				API.sendBC("§b§l" + jogador.getName() + " §fbaniu §c§lPERMANENTEMENTE §fo jogador §c"
						+ vitimaoff.getName() + " §fMotivo: §c" + Motivo);
				API.sendMsg(jogador, "§4§lBAN §fVocê baniu o jogador §c§l" + vitimaoff.getName() + "§f.");
				API.sendStaff("§4§lBAN §f" + jogador.getName() + " baniu o jogador §c" + vitimaoff.getName()
						+ "§fMotivo: §c" + Motivo);
			...
			API.sendBC("§c" + jogador.getName() + " §fbaniu §c§lPERMANENTEMENTE §fo jogador §c" + vitima.getName()
					+ " §fMotivo: §c" + Motivo);
			API.sendStaff("§4§lBAN §f" + jogador.getName() + " baniu o jogador §c" + vitima.getName() + " Motivo: §c"
					+ Motivo);
