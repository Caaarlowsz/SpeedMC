		...
		if (Cmd.getName().equalsIgnoreCase("tp")) {
			...
			if (Args.length > 2 || Args.length == 3) {
				...
				if (!CheckarNumero(Args[1])) {
					p.sendMessage(
							"§c§lSPEED§f§lMC §fÉ necessário utilizar apenas números para teleportar para uma §c§lcoordenada.");
					return true;
				}
				...
				p.sendMessage(
						"§c§lSPEED§f§lMC §7Você foi §9§lTELEPORTEADO §7até as coordenadas: §c" + X + " " + Y + " " + Z);
				return true;
			}
