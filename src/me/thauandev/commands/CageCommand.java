...
public class CageCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("cage")) {
			...
			if (args.length == 0) {
				p.sendMessage(
						"§c§lSPEED§f§lMC §7Por favor utilize §c/cage (nick) §7para prender um jogador. Lembre-se: É necessário que você seja no mínimo §3§lYOUTUBER+ §7ou §5§lMOD+ §7para executar este comando por espontânea vontade ou poderá sofrer consequências graves!");
				return true;
			}
			if (args.length == 1) {
				...
				t.sendMessage(
						"§c§lSPEED§f§lMC §fAlgum §5§lMODERADOR §fDesconfia que voçê está utilizando programas ilegais por favor permaneça na cage até segunda ordem, caso delogue será banido §c§lPERMANENTEMENTE§f!");
				return true;
			}
		}
		return false;
