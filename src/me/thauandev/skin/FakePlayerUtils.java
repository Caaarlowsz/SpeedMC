...
public class FakePlayerUtils {
	...
	public static void changePlayerSkin(final Player player, final String name, final UUID uuid,
		...
		try {
			playerProfile.getProperties().put("textures",
					(Property) CustomPlayerAPI.Textures.get((GameProfile) new GameProfile(uuid, name)));
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (respawn) {
	...
	public void addToTab(final Player player, final Collection<? extends Player> players) {
		final PacketPlayOutPlayerInfo addPlayerInfo = PacketPlayOutPlayerInfo
				.addPlayer(((CraftPlayer) player).getHandle());
	...
	public static void respawnPlayer(final Player player, final Player[] players) {
		...
		final PacketPlayOutEntityHeadRotation headRotation = new PacketPlayOutEntityHeadRotation((Entity) entityPlayer,
