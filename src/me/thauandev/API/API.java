											...
											if ((cfTempGrupo.pegargrupo().getInt("tempgrupos." + p.getUniqueId()
												...
												if ((cfTempGrupo.pegargrupo().getInt("tempgrupos." + p.getUniqueId()
													...
													if ((cfTempGrupo.pegargrupo()
														...
														API.mandarSubTitulo(p,
																"§c§lSPEED§f§lMC O seu VIP "
																		+ cfGrupo.pegargrupo().getString(
																				"grupos." + p.getUniqueId() + ".grupo")
																		+ " §7acabou de expirar!");
														cfGrupo.salvarconfiguracao();
														Bukkit.getConsoleSender()
																.sendMessage("§c§lSPEED§f§lMC §fO VIP do jogador §6§l"
																		+ p.getName() + " §facabou de expirar!");
														p.sendMessage("§c§lSPEED§f§lMC §7O seu VIP "
																+ cfGrupo.pegargrupo().getString(
																		"grupos." + p.getUniqueId() + ".grupo")
																+ "§7 acabou de expirar!");
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Nick", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".grupo", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Dia", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Mes", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Ano", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Hora", null);
														cfTempGrupo.pegargrupo()
																.set("tempgrupos." + p.getUniqueId() + ".Minuto", null);
														cfTempGrupo.pegargrupo().set(
																"tempgrupos." + p.getUniqueId() + ".Segundos", null);
														...
														for (Player todos : Bukkit.getOnlinePlayers()) {
															...
															if (cfGrupo.ChecarGrupo(todos, "Dono")
																...
																todos.sendMessage("§c§lSPEED§f§lMC §fO VIP "
																		+ cfGrupo.pegargrupo().getString(
																				"grupos." + p.getUniqueId() + ".grupo")
																		+ "§f do jogador §6§l" + p.getName()
																		+ " §facagou de expirar!");
															}
														}
														cfGrupo.setarGrupo(p, "Membro");
	...
	public static void AutomaticMessage() {
		...
		case 1:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fO servidor está em fase §1§lBETA§f. Caso você encontre algum §4§lERRO§f, reporte em nosso Twitter!");
			break;
		case 2:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fPara fazer uma denúncia, use: §c/report (nick) (motivo)");
			break;
		case 3:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fQuer saber seu status? use o comando §c/status §fpara saber seu status!");
			break;
		case 4:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fDeseja tag §b§lYT §fdigite §c/youtuber §fpara saber os requisitos");
			break;
		case 5:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fHá algum interesse em integrar à nossa §c§lEQUIPE§f, §facesse este link! §7(§chttp://bit.ly/speedmc§7)");
			break;
		case 6:
			Bukkit.broadcastMessage(
					"§c§lSPEED§f§lMC §8§l;;§f §fSiga-nos no §b§lTWITTER §fe fique por dentro de nossas novidades! Acesse: §7(§c§nhttp://twitter.com/speedmc_§7) §fou §7(§c§nhttp://twitter.com/thauanpvp§7)");
			break;
		}
	}
			...
			if (cfGrupo.ChecarGrupo(todos, "Dono") || cfGrupo.ChecarGrupo(todos, "Gerente")) {
				for (Player k : Bukkit.getOnlinePlayers()) {
					if (cfGrupo.ChecarGrupo(k, "Dono") || cfGrupo.ChecarGrupo(k, "Gerente")
							...
							|| cfGrupo.ChecarGrupo(k, "Youtuber+")) {
