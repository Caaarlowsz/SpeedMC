...
public class eUtills implements Listener {
	...
	public static long Cooldown(Player p) {
		long tempo = TimeUnit.MILLISECONDS
				.toSeconds(((Long) CooldownChat.get(p.getName())).longValue() - System.currentTimeMillis());

		if ((CooldownChat.containsKey(p.getName()))
				|| (((Long) CooldownChat.get(p.getName())).longValue() > System.currentTimeMillis())) {
			return tempo;
		}
		return 0L;
	}
	...
	public void chat(AsyncPlayerChatEvent e) {
		...
		e.setFormat(p.getDisplayName() + " §7(" + sScoreAPI.getRank(p) + "§7)" + "§f: §f"
		...
		if (cfGrupo.ChecarGrupo(p, "Dono") || cfGrupo.ChecarGrupo(p, "Gerente") || cfGrupo.ChecarGrupo(p, "Admin")
				...
				|| cfGrupo.ChecarGrupo(p, "S-Pro") || cfGrupo.ChecarGrupo(p, "Light") || cfGrupo.ChecarGrupo(p, "Pro")
	...
	public void onCommand(PlayerCommandPreprocessEvent e) {
		...
		if (e.getMessage().contains(":")) {
			if (!e.getMessage().toLowerCase().startsWith("/tell")) {
				if (!cfGrupo.ChecarGrupo(p, "Dono") && !cfGrupo.ChecarGrupo(p, "Mod+") && !cfGrupo.ChecarGrupo(p, "Mod")
					...
					e.setCancelled(true);
					p.sendMessage(
							"§c§lSPEED§f§lMC §cVocê não pode utilizar comandos que possuam §c§l: §c(dois pontos)");
				}
			}
		}
	}
			...
			for (Entity entidades : e.getPlayer().getNearbyEntities(100, 150, 100)) {
				...
				if ((entidades instanceof Player)
					...
					e.getPlayer().sendMessage(
							"§c§lSPEED§f§lMC §7Bússula apontando para §c" + ((Player) entidades).getName());
					return;
				}
			}
			if (!parar) {
